...
const QString REQUEST_URL =
    QStringLiteral("http://data.fixer.io/api/latest?access_key=") %
    FIXER_EXCHANGES_API_KEY;
} // namespace
...
CurrencyService::CurrencyService(QObject* parent)
    : QObject(parent), _refreshTimer{this}, _net{this}
{
    connect(&_net, &QNetworkAccessManager::finished, this,
            &CurrencyService::processReply);
    connect(&_refreshTimer, &QTimer::timeout, this,
            &CurrencyService::requestExchangeRates);
...
void CurrencyService::processReply(QNetworkReply* reply)
{
    ...
    if (reply->error() != QNetworkReply::NoError)
    {
        qDebug()
            << QStringLiteral(
                   "error received while processing exchange rates reply: ")
            << reply->error();
        return;
    }
    ...
    if (code != 200)
    {
        qDebug() << QStringLiteral("response for exchange rates returned ")
                 << code;
        return;
    }
