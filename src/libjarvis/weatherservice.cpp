...
WeatherInfo parseCurrentWeatherData(const QByteArray& data)
{
    ...
    WeatherInfo info;
    info.temperature =
        MeasurementHelpers::parseMeasurement(QString::number(temp));
...
QList<ForecastInfo> parseForecastData(const QByteArray& data)
{
    ...
    const auto forecast1 = ForecastInfo{
        MeasurementHelpers::parseMeasurement(QString::number(maxTempDay1)),
        maxWeatherDay1,
        MeasurementHelpers::parseMeasurement(QString::number(minTempDay1)),
        minWeatherDay1};
    const auto forecast2 = ForecastInfo{
        MeasurementHelpers::parseMeasurement(QString::number(maxTempDay2)),
        maxWeatherDay2,
        MeasurementHelpers::parseMeasurement(QString::number(minTempDay2)),
        minWeatherDay2};
    const auto forecast3 = ForecastInfo{
        MeasurementHelpers::parseMeasurement(QString::number(maxTempDay3)),
        maxWeatherDay3,
        MeasurementHelpers::parseMeasurement(QString::number(minTempDay3)),
        minWeatherDay3};
...
void WeatherService::setForecast(const QList<ForecastInfo>& info)
{
    ...
    for (const auto& f : info)
        _forecast << QVariant::fromValue(new ForecastWeather{f, this});
